...
import sqlite3
import psutil
from security_core import EnhancedUSBSecurity

class EnhancedUSBSecurityApp:
    def __init__(self, root):
        self.root = root
        self.security = EnhancedUSBSecurity()
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        self.root.title("Enhanced USB Physical Security System v2.0")
    ...
    def setup_main_control(self):
        ...
        # Title
        title_label = tk.Label(self.main_frame, text="Enhanced USB Physical Security v2.0", 
    ...
    def setup_settings(self):
        ...
        tk.Checkbutton(sec_frame, text="Auto-block unauthorized USB devices", 
                      variable=self.auto_block_var, command=self.save_security_settings).pack()
        ...
        tk.Checkbutton(sec_frame, text="Enable device whitelist", 
                      variable=self.whitelist_var, command=self.save_security_settings).pack()
        
        self.audit_var = tk.BooleanVar(value=self.security.config["audit_logging"])
        tk.Checkbutton(sec_frame, text="Enable audit logging", 
                      variable=self.audit_var, command=self.save_security_settings).pack()
        ...
        tk.Button(timeout_frame, text="Save", command=self.save_security_settings).pack(side=tk.LEFT)
    
    def setup_audit_log(self):
        """Setup audit log interface"""
        # Create treeview for audit log
        columns = ("Timestamp", "Action", "User", "Details", "IP")
    ...
    def setup_dashboard(self):
        ...
        # Statistics frame
        stats_frame = tk.LabelFrame(self.dashboard_frame, text="📈 System Statistics", 
                                   font=("Arial", 12, "bold"), fg="#00ff00")
        stats_frame.pack(fill='x', padx=20, pady=10)
        
        self.stats_text = tk.Text(stats_frame, height=20, width=70, bg="#2d2d2d", fg="#ffffff")
        self.stats_text.pack(pady=10)
        
        # Refresh button
        tk.Button(self.dashboard_frame, text="🔄 Refresh Statistics", 
    ...
    def change_password(self):
        ...
        if new_password:
            ...
            if new_password == confirm_password:
                self.security.config["admin_password"] = self.security.encrypt_password(new_password)
                self.security.save_config()
                self.security.log_audit_event("Password Changed", "Admin password updated")
    ...
    def save_security_settings(self):
        ...
        self.security.save_config()
        self.security.log_audit_event("Settings Updated", "Security settings modified")
    ...
    def refresh_audit_log(self):
        ...
        cursor = conn.cursor()
        cursor.execute('SELECT timestamp, action, user, details, ip_address FROM audit_log ORDER BY timestamp DESC LIMIT 100')
    ...
    def add_current_device(self):
        ...
        self.security.log_audit_event("Device Whitelisted", f"Added device: {device_id}")
        self.refresh_whitelist()
    ...
    def remove_whitelist_device(self):
        ...
        self.security.log_audit_event("Device Removed from Whitelist", f"Removed device: {device_id}")
        self.refresh_whitelist()
...
=== USB SECURITY STATUS ===
USB Status: {'ENABLED' if self.usb_enabled else 'DISABLED'}
Auto-block: {'ENABLED' if self.security.config['auto_block'] else 'DISABLED'}
Whitelist: {'ENABLED' if self.security.config['whitelist_enabled'] else 'DISABLED'}
Audit Logging: {'ENABLED' if self.security.config['audit_logging'] else 'DISABLED'}
...
"""
        ...
        for drive in drives:
            try:
                ...
                stats += f"{drive}: {usage.used // (1024**3)}GB / {usage.total // (1024**3)}GB ({usage.percent}%)\n"
            except:
                stats += f"{drive}: Access Denied\n"
...
- Admin Rights: {'Yes' if self.is_admin() else 'No'}
    ...
    def disable_usb_ports(self):
        ...
        try:
            ...
            if self.security.config["audit_logging"]:
                self.security.log_audit_event("USB Disabled", "USB storage ports disabled")
            return True
        except Exception as e:
            if self.security.config["audit_logging"]:
                self.security.log_audit_event("USB Disable Failed", str(e))
            return False
    ...
    def enable_usb_ports(self):
        ...
        try:
            ...
            winreg.CloseKey(reg_key)
            if self.security.config["audit_logging"]:
                self.security.log_audit_event("USB Enabled", "USB storage ports enabled")
            return True
        except Exception as e:
            if self.security.config["audit_logging"]:
                self.security.log_audit_event("USB Enable Failed", str(e))
            return False
    ...
    def authenticate_user(self):
        ...
        if not self.security.verify_password(password):
            ...
            if self.security.config["audit_logging"]:
                self.security.log_audit_event("Authentication Failed", "Incorrect password entered")
            return False
        
        if self.security.config["audit_logging"]:
            self.security.log_audit_event("Authentication Success", "Correct password entered")
        return True
    ...
    def handle_new_device(self, new_drives):
        ...
        if self.security.config["audit_logging"]:
            self.security.log_audit_event("USB Device Detected", f"New devices: {device_list}")
...
def main():
    ...
    root = tk.Tk()
    app = EnhancedUSBSecurityApp(root)
